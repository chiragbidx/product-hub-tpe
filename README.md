...
Production-oriented **Next.js App Router** starter with typed content contracts, reusable UI primitives, pnpm-based builds, and container/runtime automation scripts.
...
Treat these as placeholders for future database onboarding, not active runtime requirements for the current UI app.
...
- `DATABASE_URL` (future DB flow via `scripts/db-init.js`)
